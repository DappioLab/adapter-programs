...
use crate::msg_panic;
use crate::common::math::decimal::Decimal;
use crate::common::math::uint::U192;
use crate::common::{
    math,
    traits::{
...
impl TokenizedShares for VaultBaseV1 {
    ...
    fn exchange_rate(&mut self, mint: &Mint) -> f64 {
        // sync shares first
        self.sync_shares(mint);
        let total_deposited_balance =
            amount_to_ui_amount(self.total_deposited_balance, mint.decimals);
        let total_shares = amount_to_ui_amount(self.total_shares, mint.decimals);
